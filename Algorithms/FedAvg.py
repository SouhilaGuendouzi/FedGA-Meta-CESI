...
print("Starting FedAVG framework...")
...
fog_1 = Fog(1, model_mnist, [(train_loaders1[i], test_loaders1[i]) for i in range(args.num_users)], optimizer_mnist, loss_fn, copy.deepcopy(args),'Fed_avg')
fogs.append(fog_1)
...
fog_2 = Fog(2, model_usps, [(train_loaders2[i], test_loaders2[i]) for i in range(args.num_users)], optimizer_usps, loss_fn, copy.deepcopy(args),'Fed_avg')
fogs.append(fog_2)
...
fog_3 = Fog(3, model_svhn, [(train_loaders3[i], test_loaders3[i]) for i in range(args.num_users)], optimizer_svhn, loss_fn, copy.deepcopy(args),'Fed_avg')
fogs.append(fog_3)
...
fog_4 = Fog(4, model_mnistm, [(train_loaders4[i], test_loaders4[i]) for i in range(args.num_users)], optimizer_mnistm, loss_fn, copy.deepcopy(args),'Fed_avg')
fogs.append(fog_4)
...
fog_5 = Fog(5, model_emnist, [(train_loaders5[i], test_loaders5[i]) for i in range(args.num_users)], optimizer_emnist, loss_fn, copy.deepcopy(args),'Fed_avg')
fogs.append(fog_5)



### Exécution de FedAvg pour chaque Fog ###

for fog in fogs:
    print(f"Starting FedAvg for Fog {fog.id}")
    fog.federated_training()
    print(f"Completed FedAvg for Fog {fog.id}")

print("FedAvg training completed for all Fogs.")
