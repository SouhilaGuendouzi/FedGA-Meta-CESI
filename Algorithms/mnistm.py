...
def get_mnistm_distribution(type="iid", n_samples_train=500, n_samples_test=100, n_clients=3, batch_size=25, shuffle=True, alpha=0.5):
       
    dataset_loaded_train=MNISTM(root='./data', train=True, download=True, transform=transforms.Compose([transforms.ToTensor()]))
    dataset_loaded_test=MNISTM(root='./data', train=False, download=True, transform=transforms.Compose([transforms.ToTensor()]))
    global_dataset = torch.utils.data.ConcatDataset([dataset_loaded_train, dataset_loaded_test])
